...
$rate = Sec::tu($rate);
$product = Sec::N($_REQUEST['product']);
if($rate != "" && substr($rate,0,3) != "ERR"){
	//die($rate);
	if($product != "" && substr($product,0,3) != "ERR"){
	$usb = new DB_Act("I","(NULL, '".$product."feed', '".$rate."', '".time()."')","nim_ratings"," (`id`, `product`, `rate`, `time`)");
	}else{
	$usb = new DB_Act("I","(NULL, 'zuoyefeed', '".$rate."', '".time()."')","nim_ratings"," (`id`, `product`, `rate`, `time`)");	
	}
...
$( document ).on( "click", "#rate-up", function() {
			...
			$.ajax("rate.php?product="+product+"&rate="+$("#rate")[0].value).done(on_or);			
			var $this = $( this ),
