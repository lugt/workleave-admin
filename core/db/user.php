...
class BPUB {
	...
	public function G_ID($uid){
		...
		// SELECT
		//echo("FUCKIT");
		$this->db_c = new DB_Act("S","*","bwfx_teacher"," uid = '".$uid."'");
		//die($this->db_c->get);
	...
	public function G_USN($usn){
		$this->shift_user = new User_conf();
		$this->db_c = new DB_Act("S","*","bwfx_teacher"," usn = '".$usn."'");
		//die($this->db_c->get);
		if(strlen($this->db_c->get['usn']) >= 2){
			$this->G_CONF();
			return $this->shift_user;
	...
	public function G_SS($ses){
		...
		// 合法性验证完毕
		// SELECT
		$this->db_c = new DB_Act("S","*","bwfx_teacher"," sess = '".$ses."'");
		if(strlen($this->db_c->get['usn']) >= 2){
			$this->G_CONF();
			return $this->shift_user;
		}else{
	...
	public function S($usn,$thing){
		...
		// 合法性验证完毕
		// UPDATE
		//$this->db_c = new DB_Act("U","*","bwfx_teacher"," id = '".$this->shift_user->uid."'");
		$this->db_c = new DB_Act("U","`".$usn."` =  '".$thing."' ","bwfx_teacher"," `uid` = ".$this->shift_user->uid."");
		//print_r($dnsdb);
		//echo($dnsdb->get);
